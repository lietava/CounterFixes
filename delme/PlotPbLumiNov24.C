...
{ // "http://ccdb-test.cern.ch:8080"
  ...
  double_t lumiZNC = recs[recs.size() - 1].scalers[zncc].l1Before - recs[0].scalers[zncc].l1Before;
  double_t lumiTCE = recs[recs.size() - 1].scalers[tce].lmBefore - recs[0].scalers[tce].lmBefore;
  ...
  std::cout << " Run duration:" << Trun << " Scalers size:" << n + 1 << " ZNC lumi (no pileupcorr):" << lumiZNC/28 << " TCE lumi:" << lumiTCE << std::endl;
