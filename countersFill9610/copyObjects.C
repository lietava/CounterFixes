...
void copyObjects(bool cfg =0, bool scl = 0, bool writeToCCDB = false ) {
  ...
  std::cout << "Number of scalers only tested (NOT written to ccdb):" << isclt << " written to CCDB:" << isclw << std::endl;
  std::cout << "Number of configs only tested (NOT written to ccdb):" << icfgt << " written to CCDB:" << icfgw << std::endl;
  return;
}
