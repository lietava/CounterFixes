...
void GetAndSave(std::string ccdbHost = "http://ccdb-test.cern.ch:8080")
{
  ...
  std::vector<string> runs = {"539700"};
  std::vector<long> timestamps = {1689581000000};
  int i = 0;
  CTPRunManager mng;
  ...
  for (auto const& run : runs) {
    ...
    if (doscalers) {
      ...
      if (ctpscalers == nullptr) {
        ...
        std::string name = "CTPScalers" + run + ".root";
    ...
    if ( doconfig) {  
      CTPConfiguration* ctpconfig = mgr.getSpecific<CTPConfiguration>(CCDBPathCTPConfig, timestamps[i], metadata);
      if (ctpconfig == nullptr) {
        std::cout << run << " CTPConfig not in database, timestamp:" << timestamps[i] << std::endl;
      } else {
        // ctpscalers->printStream(std::cout);
        std::string name = "CTPConfig" + run + ".root";
        TFile* myFile = TFile::Open(name.c_str(), "RECREATE");
        myFile->WriteObject(ctpconfig, "CTPConfig");
        // myFile->Write();
        std::cout << run << " ok" << std::endl;
      }
    }
