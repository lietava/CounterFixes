...
bool inEtaAcceptance(double eta, double etaCut)
{
  return eta > -etaCut && eta < etaCut;
}
...
TString dirNameFromPath(const TString& path)
{
  TString dir = path;
...
double readCfgDownscaleMB(const TString& ao2dPath, bool& found)
{
  found = false;
  const TString configPath = dirNameFromPath(ao2dPath) + "/full_config.json";
  std::ifstream input(configPath.Data());
  if (!input) {
    std::cerr << "Cannot open downscale config: " << configPath << std::endl;
    return 1.0;
  }
  ...
  const std::string key = "\"cfgDownscaleMB\"";
  const std::size_t keyPos = text.find(key);
  if (keyPos == std::string::npos) {
    std::cerr << "cfgDownscaleMB not found in " << configPath << std::endl;
    return 1.0;
  }

  const std::size_t colonPos = text.find(':', keyPos + key.size());
  if (colonPos == std::string::npos) {
    std::cerr << "Malformed cfgDownscaleMB entry in " << configPath << std::endl;
    return 1.0;
  }

  const std::size_t valueStart = text.find_first_of("\"0123456789.-+", colonPos + 1);
  if (valueStart == std::string::npos) {
    std::cerr << "Missing cfgDownscaleMB value in " << configPath << std::endl;
    return 1.0;
  }
  ...
  if (text[valueStart] == '"') {
    ++valueEnd;
    const std::size_t quoteEnd = text.find('"', valueEnd);
    if (quoteEnd == std::string::npos) {
      std::cerr << "Unterminated cfgDownscaleMB string in " << configPath << std::endl;
      return 1.0;
    }
    const std::string value = text.substr(valueEnd, quoteEnd - valueEnd);
    found = true;
    return std::atof(value.c_str());
  }

  while (valueEnd < text.size() &&
         (std::isdigit(text[valueEnd]) || text[valueEnd] == '.' ||
...
std::map<int, double, std::greater<int>> buildGeneratedFT0EventWeights(TChain& chain)
{
  std::map<int, double, std::greater<int>> weights;
  if (!chain.GetBranch("fMultGen") || !chain.GetBranch("fMultGenFT0")) {
    return weights;
  }

  int multGen = 0;
  int multGenFT0 = 0;
  chain.SetBranchAddress("fMultGen", &multGen);
  chain.SetBranchAddress("fMultGenFT0", &multGenFT0);
...
double generatedFT0Centrality(int multGenFT0,
                              const std::map<int, double>& centrality)
{
  if (multGenFT0 < 0 || centrality.empty()) {
    return -1.0;
  }
  const auto it = centrality.find(multGenFT0);
                                           ...
                                           Matrix2D_t& RecoAllTrain)
{
  ...
  std::cout << "  eta acceptance: |eta| < " << etaCut
            << " rows with pt-in-range eta rejection: " << nEtaRejected << std::endl;
                          ...
                          TH1D* hPtRecoData)
{
  ...
  for (auto key : *file.GetListOfKeys()) {
    ...
    if (!candTree->GetBranch("fPtRec") ||
        !candTree->GetBranch("fEtaRec") ||
        !candTree->GetBranch("fIndexNPCollisionTable")) {
      std::cerr << "NPRecoChargedCand in " << keyName
                << " is missing required branches fPtRec, fEtaRec and/or fIndexNPCollisionTable" << std::endl;
      return false;
    }
    ...
    candTree->SetBranchAddress("fEtaRec", &etaReco);
  ...
  std::cout << "  reco candidates filled: " << nFilled
            << " ignored: " << nIgnored << std::endl;
  std::cout << "  eta acceptance: |eta| < " << etaCut
            << " reco candidates rejected by eta: " << nEtaRejected << std::endl;
                             ...
                             TH1D* hPtTrueData)
{
  ...
  if (!truthChain.GetBranch("fPtGen") ||
      !truthChain.GetBranch("fEtaGen") ||
      !truthChain.GetBranch("fMultGen") ||
      !truthChain.GetBranch("fMultGenFT0")) {
    std::cerr << "NPMCChargedTABLE is missing required branches fPtGen, fEtaGen, fMultGen and/or fMultGenFT0" << std::endl;
    return false;
  }

  const auto generatedCentLookup = buildGeneratedFT0CentralityLookup(buildGeneratedFT0EventWeights(truthChain));
  ...
  truthChain.SetBranchAddress("fEtaGen", &etaTrue);
  truthChain.SetBranchAddress("fMultGen", &multTrue);
  truthChain.SetBranchAddress("fMultGenFT0", &multGenFT0);

  for (Long64_t i = 0; i < truthChain.GetEntries(); ++i) {
    truthChain.GetEntry(i);
    const double centGenFT0 = generatedFT0Centrality(multGenFT0, generatedCentLookup);
    if (centGenFT0 < centMin || centGenFT0 >= centMax) {
      ++nIgnored;
      continue;
    }
    if (ptTrue >= ptMin && ptTrue < ptMax &&
  ...
  std::cout << "  generated FT0 centrality accepted range: [" << centMin << ", " << centMax << ")" << std::endl;
  std::cout << "  truth particles filled: " << nTruth
            << " ignored: " << nIgnored << std::endl;
  std::cout << "  eta acceptance: |eta| < " << etaCut
            << " truth particles rejected by eta: " << nEtaRejected << std::endl;
  return true;
}
                                    ...
                                    TH1D* hPtRecoData)
{
  ...
  if (useMeasuredCentFT0M && !dataChain.GetBranch("fCentFT0M")) {
    std::cerr << "NPMCChargedTABLE is missing required branch fCentFT0M for measured-centrality closure" << std::endl;
    return false;
  }

  const auto generatedCentLookup = buildGeneratedFT0CentralityLookup(buildGeneratedFT0EventWeights(dataChain));
  ...
  if (useMeasuredCentFT0M) {
    dataChain.SetBranchAddress("fCentFT0M", &centFT0M);
  }
  ...
  for (Long64_t i = 0; i < dataChain.GetEntries(); ++i) {
    ...
    if (recoPtInRange && !inEtaAcceptance(etaReco, etaCut)) {
      ++nRecoEtaRejected;
    }
  ...
  std::cout << "  " << (useMeasuredCentFT0M ? "measured fCentFT0M" : "generated FT0")
            << " centrality accepted range: [" << centMin << ", " << centMax << ")" << std::endl;
  std::cout << "  truth particles filled: " << nTruth
            << " reco rows filled: " << nReco
            << " ignored rows: " << nIgnored << std::endl;
  std::cout << "  eta acceptance: |eta| < " << etaCut
            << " truth eta rejected: " << nTruthEtaRejected
            << " reco eta rejected: " << nRecoEtaRejected << std::endl;
                ...
                double etaCut = 0.8, int nIterInput = 10)
{
  ...
  if (etaCut <= 0.0) {
    std::cerr << "Invalid etaCut = " << etaCut
  ...
  const double etaWidth = 2.0 * etaCut;
  ...
  if (useAO2Data == 1 || useAO2Data == 3) {
    ...
    if (!readRecoDataFromAO2D(dataAO2D, ptMin, ptMax, multMin, multMax, etaCut,
    ...
    if (useAO2Data == 3) {
      ...
      if (!readTruthDataFromMCAO2D(truthAO2D, ptMin, ptMax, multMin, multMax, etaCut,
                                   centMin, centMax,
                                   hTruthDataTmp, hMultTrueData, hPtTrueData)) {
        return;
      }
    }
  } else if (useAO2Data == 4 || useAO2Data == 5) {
    if (sameBCPolicy != 0) {
      std::cout << "sameBCPolicy requested, but useAO2Data=" << useAO2Data << " reads O2npmcchargedtabl, "
    ...
    std::cerr << "Unknown useAO2Data option " << useAO2Data
              << ": use 1 for reco data, 0 for generated data, 2 for RecoTrain, 3 for MC2 reco+truth closure, 4 for MC-table generated-cent closure, 5 for MC-table measured-cent closure" << std::endl;
    return;
  }
  ...
  double truthClosureScale = 1.0;
  if (useAO2Data == 3) {
    bool foundDataDownscale = false;
    bool foundTruthDownscale = false;
    dataDownscaleMB = readCfgDownscaleMB(dataAO2D, foundDataDownscale);
    truthDownscaleMB = readCfgDownscaleMB(truthAO2D, foundTruthDownscale);
    if (foundDataDownscale && foundTruthDownscale &&
        dataDownscaleMB > 0.0 && truthDownscaleMB > 0.0) {
      truthClosureScale = truthDownscaleMB / dataDownscaleMB;
      std::cout << "Mode 3 downscale normalization:"
                << " data cfgDownscaleMB=" << dataDownscaleMB
                << " truth cfgDownscaleMB=" << truthDownscaleMB
                << " truthClosureScale=" << truthClosureScale << std::endl;
      for (int it = 0; it < NPT; ++it) {
        for (int jt = 0; jt < NMULT; ++jt) {
          TruthData[it][jt] *= truthClosureScale;
        }
      }
      hPtTrueData->Scale(truthClosureScale);
      hMultTrueData->Scale(truthClosureScale);
    } else {
      std::cerr << "Mode 3 downscale normalization skipped; using unscaled truth closure reference." << std::endl;
      dataDownscaleMB = foundDataDownscale ? dataDownscaleMB : 1.0;
      truthDownscaleMB = foundTruthDownscale ? truthDownscaleMB : 1.0;
      truthClosureScale = 1.0;
    }
  }
  ...
  TH2D* hEfficiency = new TH2D("hEfficiency",
  ...
  TH2D* hResponsePtProj = new TH2D("hResponsePtProj",
                                   "Response projected to p_{T};reco p_{T};truth p_{T}",
                                   NPT, ptMin, ptMax, NPT, ptMin, ptMax);
  TH2D* hResponseMultProj = new TH2D("hResponseMultProj",
                                     "Response projected to multiplicity;reco mult;truth mult",
                                     NMULT, multMin, multMax, NMULT, multMin, multMax);
  TH2D* hResponseCountPtProj = new TH2D("hResponseCountPtProj",
                                        "Response counts projected to p_{T};reco p_{T};truth p_{T}",
                                        NPT, ptMin, ptMax, NPT, ptMin, ptMax);
  TH2D* hResponseCountMultProj = new TH2D("hResponseCountMultProj",
                                          "Response counts projected to multiplicity;reco mult;truth mult",
                                          NMULT, multMin, multMax, NMULT, multMin, multMax);
  ...
  for (int it = 0; it < NPT; ++it) {
    for (int jt = 0; jt < NMULT; ++jt) {
      ...
      hEfficiency->SetBinContent(it + 1, jt + 1, effs[it][jt]);
      const double truth = TruthTrain[it][jt];
      const double effClamped = std::clamp(effs[it][jt], 0.0, 1.0);
      const double effErr = truth > 0.0 ? std::sqrt(effClamped * (1.0 - effClamped) / truth) : 0.0;
      hEfficiency->SetBinError(it + 1, jt + 1, effErr);
    }
  }
    ...
    for (int jr = 0; jr < NMULT; ++jr) {
      ...
      for (int it = 0; it < NPT; ++it) {
        for (int jt = 0; jt < NMULT; ++jt) {
          ...
          hResponsePtProj->AddBinContent(hResponsePtProj->GetBin(ir + 1, it + 1),
                                         Rprob[ir][jr][it][jt]);
          hResponseMultProj->AddBinContent(hResponseMultProj->GetBin(jr + 1, jt + 1),
                                           Rprob[ir][jr][it][jt]);
          hResponseCountPtProj->AddBinContent(hResponseCountPtProj->GetBin(ir + 1, it + 1),
                                              Rcount[ir][jr][it][jt]);
          hResponseCountMultProj->AddBinContent(hResponseCountMultProj->GetBin(jr + 1, jt + 1),
                                                Rcount[ir][jr][it][jt]);
        }
      }
    }
  ...
  TH1D* hMatchedTrainPtProj = hResponseCountPtProj->ProjectionY("hMatchedTrainPtProj");
  hMatchedTrainPtProj->SetTitle("Training matched truth p_{T};p_{T};entries");
  TH1D* hEfficiencyPtProj = (TH1D*)hMatchedTrainPtProj->Clone("hEfficiencyPtProj");
  hEfficiencyPtProj->SetTitle("Matched reconstruction efficiency;p_{T};matched / truth");
  hEfficiencyPtProj->Divide(hMatchedTrainPtProj, hTruthTrainPtProj, 1.0, 1.0, "B");
  hEfficiencyPtProj->SetOption("E1");
  TH1D* hLostSignalPtProj = (TH1D*)hMissPtProj->Clone("hLostSignalPtProj");
  hLostSignalPtProj->SetTitle("Lost signal;p_{T};missed truth signal");
  TH1D* hLostSignalFractionPtProj = (TH1D*)hLostSignalPtProj->Clone("hLostSignalFractionPtProj");
  hLostSignalFractionPtProj->SetTitle("Lost signal fraction;p_{T};missed / truth");
  hLostSignalFractionPtProj->Divide(hLostSignalPtProj, hTruthTrainPtProj, 1.0, 1.0, "B");
  hLostSignalFractionPtProj->SetOption("E1");
  ...
  TH1D* hMatchedTrainMultProj = hResponseCountMultProj->ProjectionY("hMatchedTrainMultProj");
  hMatchedTrainMultProj->SetTitle("Training matched truth mult;mult;entries");
  TH1D* hEfficiencyMultProj = (TH1D*)hMatchedTrainMultProj->Clone("hEfficiencyMultProj");
  hEfficiencyMultProj->SetTitle("Matched reconstruction efficiency;mult;matched / truth");
  hEfficiencyMultProj->Divide(hMatchedTrainMultProj, hTruthTrainMultProj, 1.0, 1.0, "B");
  hEfficiencyMultProj->SetOption("E1");
  ...
  auto makeDNdetaProj = [etaWidth](TH1D* source, const char* name, const char* title) {
    std::vector<double> edges(source->GetNbinsX() + 1, 0.0);
    for (int ib = 1; ib <= source->GetNbinsX(); ++ib) {
      edges[ib - 1] = source->GetXaxis()->GetBinLowEdge(ib) / etaWidth;
    }
    edges[source->GetNbinsX()] = source->GetXaxis()->GetBinUpEdge(source->GetNbinsX()) / etaWidth;

    TH1D* h = new TH1D(name, title, source->GetNbinsX(), edges.data());
    h->Sumw2();
    for (int ib = 1; ib <= source->GetNbinsX(); ++ib) {
      h->SetBinContent(ib, source->GetBinContent(ib));
      h->SetBinError(ib, source->GetBinError(ib));
    }
    return h;
  };

  TH1D* hTruthTrainDNdetaProj = makeDNdetaProj(hTruthTrainMultProj,
                                               "hTruthTrainDNdetaProj",
                                               "TruthTrain central multiplicity;dN/d#eta;entries");
  TH1D* hTruthDataDNdetaProj = makeDNdetaProj(hTruthDataMultProj,
                                              "hTruthDataDNdetaProj",
                                              "TruthData central multiplicity;dN/d#eta;entries");
  TH1D* hUnfoldDNdetaProj = makeDNdetaProj(hUnfoldMultProj,
                                           "hUnfoldDNdetaProj",
                                           "Unfold central multiplicity;dN/d#eta;entries");

  TH1D* hMissMultProj       = hMiss->ProjectionY("hMissMultProj");
  TH1D* hLostSignalMultProj = (TH1D*)hMissMultProj->Clone("hLostSignalMultProj");
  hLostSignalMultProj->SetTitle("Lost signal;mult;missed truth signal");
  TH1D* hLostSignalFractionMultProj = (TH1D*)hLostSignalMultProj->Clone("hLostSignalFractionMultProj");
  hLostSignalFractionMultProj->SetTitle("Lost signal fraction;mult;missed / truth");
  hLostSignalFractionMultProj->Divide(hLostSignalMultProj, hTruthTrainMultProj, 1.0, 1.0, "B");
  hLostSignalFractionMultProj->SetOption("E1");
  ...
  TH1D* hLostCollisionMultProj = (TH1D*)hMultRecoData->Clone("hLostCollisionMultProj");
  hLostCollisionMultProj->SetTitle("Lost collisions;measured mult;all accepted collisions - collisions with selected candidates");
  TH1D* hCollisionEfficiencyMultProj = (TH1D*)hMultRecoDataCollWithCand->Clone("hCollisionEfficiencyMultProj");
  hCollisionEfficiencyMultProj->SetTitle("Collision selection efficiency;measured mult;collisions with candidates / all collisions");
  TH1D* hLostCollisionFractionMultProj = (TH1D*)hLostCollisionMultProj->Clone("hLostCollisionFractionMultProj");
  hLostCollisionFractionMultProj->SetTitle("Lost collision fraction;measured mult;lost collisions / all collisions");
  for (int ib = 1; ib <= hLostCollisionMultProj->GetNbinsX(); ++ib) {
    const double all = hMultRecoData->GetBinContent(ib);
    const double kept = hMultRecoDataCollWithCand->GetBinContent(ib);
    const double lost = std::max(0.0, all - kept);
    const double effColl = all > 0.0 ? kept / all : 0.0;
    const double lostFrac = all > 0.0 ? lost / all : 0.0;

    hLostCollisionMultProj->SetBinContent(ib, lost);
    hLostCollisionMultProj->SetBinError(ib, lost > 0.0 ? std::sqrt(lost) : 0.0);
    hCollisionEfficiencyMultProj->SetBinContent(ib, effColl);
    hCollisionEfficiencyMultProj->SetBinError(ib, all > 0.0 ? std::sqrt(effColl * (1.0 - effColl) / all) : 0.0);
    hLostCollisionFractionMultProj->SetBinContent(ib, lostFrac);
    hLostCollisionFractionMultProj->SetBinError(ib, all > 0.0 ? std::sqrt(lostFrac * (1.0 - lostFrac) / all) : 0.0);
  }
  hCollisionEfficiencyMultProj->SetOption("E1");
  hLostCollisionFractionMultProj->SetOption("E1");
  ...
  auto makeClosureRatio = [](TH1D* numerator,
                             TH1D* denominator,
                             const char* name,
                             const char* title,
    ...
    for (int ib = 1; ib <= ratio->GetNbinsX(); ++ib) {
      const double den = denominator->GetBinContent(ib);
      if (den < minDenominator) {
        ratio->SetBinContent(ib, 0.0);
        ratio->SetBinError(ib, 0.0);
        continue;
      }

      const double num = numerator->GetBinContent(ib);
      const double numErr = numerator->GetBinError(ib);
      const double denErr = denominator->GetBinError(ib);
      const double value = num / den;
      double error = 0.0;
      if (num > 0.0) {
        error = value * std::hypot(numErr / num, denErr / den);
      }
      ratio->SetBinContent(ib, value);
      ratio->SetBinError(ib, error);
    }
    return ratio;
  };

  auto restrictToSupportedRange = [](TH1D* ratio,
                                     TH1D* denominator,
                                     double minDenominator) {
    int first = 0;
    int last = 0;
    for (int ib = 1; ib <= denominator->GetNbinsX(); ++ib) {
      if (denominator->GetBinContent(ib) >= minDenominator) {
        if (first == 0) {
          first = ib;
        }
        last = ib;
    ...
    if (first > 0 && last >= first) {
      ratio->GetXaxis()->SetRange(first, last);
    }
  };

  const double minTruthPtForRatio = 1.0;
  const double minTruthMultForRatio = 1000.0;
  TH1D* hRatioPt = makeClosureRatio(hUnfoldPtProj, hTruthDataPtProj,
                                    "hRatioPt",
                                    "Unfold / Truth p_{T};p_{T};unfold / truth",
                                    minTruthPtForRatio);
  TH1D* hRatioMult = makeClosureRatio(hUnfoldMultProj, hTruthDataMultProj,
                                      "hRatioMult",
                                      "Unfold / Truth mult;mult;unfold / truth",
                                      minTruthMultForRatio);
  TH1D* hRatioMultEvent = makeClosureRatio(hUnfoldMultEventProj, hTruthDataMultEventProj,
                                           "hRatioMultEvent",
                                           "Unfold / Truth event-style mult;mult;unfold / truth",
                                           minTruthMultForRatio);
  TH1D* hRatioDNdeta = makeClosureRatio(hUnfoldDNdetaProj, hTruthDataDNdetaProj,
                                        "hRatioDNdeta",
                                        "Unfold / Truth central multiplicity;dN/d#eta;unfold / truth",
                                        minTruthMultForRatio);
  restrictToSupportedRange(hRatioPt, hTruthDataPtProj, minTruthPtForRatio);
  restrictToSupportedRange(hRatioMult, hTruthDataMultProj, minTruthMultForRatio);
  ...
  auto printAverageMult = [etaWidth](const TH1D* h, const char* label) {
    ...
    const double meanDNdeta = entries > 0.0 ? meanMult / etaWidth : 0.0;
    std::cout << "Average central multiplicity " << label
              << ": <mult>=" << meanMult
              << " <dN/deta>=" << meanDNdeta
              << " etaWidth=" << etaWidth
              << " entries=" << entries << std::endl;
  };
  printAverageMult(hTruthDataMultProj, "truth data");
  printAverageMult(hUnfoldMultProj, "unfolded");
  printAverageMult(hRecoDataMultProj, "reco data");
  ...
  TString optionsText;
  optionsText += TString::Format("trainAO2D=%s\n", trainAO2D.Data());
  optionsText += TString::Format("useAO2DTraining=%d\n", static_cast<int>(useAO2DTraining));
  optionsText += TString::Format("dataAO2D=%s\n", dataAO2D.Data());
  optionsText += TString::Format("truthAO2D=%s\n", truthAO2D.Data());
  optionsText += TString::Format("useAO2Data=%d\n", useAO2Data);
  optionsText += TString::Format("priorMode=%d\n", priorMode);
  optionsText += TString::Format("subtractFakeFeed=%d\n", static_cast<int>(subtractFakeFeed));
  optionsText += TString::Format("centMin=%g\n", centMin);
  optionsText += TString::Format("centMax=%g\n", centMax);
  optionsText += TString::Format("sameBCPolicy=%d\n", sameBCPolicy);
  optionsText += TString::Format("etaCut=%g\n", etaCut);
  optionsText += TString::Format("etaWidth=%g\n", etaWidth);
  optionsText += TString::Format("dataDownscaleMB=%g\n", dataDownscaleMB);
  optionsText += TString::Format("truthDownscaleMB=%g\n", truthDownscaleMB);
  optionsText += TString::Format("truthClosureScale=%g\n", truthClosureScale);
  optionsText += TString::Format("ptMin=%g\n", ptMin);
  optionsText += TString::Format("ptMax=%g\n", ptMax);
  optionsText += TString::Format("multMin=%g\n", multMin);
  optionsText += TString::Format("multMax=%g\n", multMax);
  optionsText += TString::Format("nIter=%d\n", nIter);
  optionsText += TString::Format("nTrainEvents=%d\n", nTrainEvents);
  ...
  hResponseCountPtProj->Write();
  hResponseCountMultProj->Write();
  ...
  hMatchedTrainPtProj->Write();
  hEfficiencyPtProj->Write();
  hLostSignalPtProj->Write();
  hLostSignalFractionPtProj->Write();
  ...
  hTruthTrainDNdetaProj->Write();
  hTruthDataDNdetaProj->Write();
  hUnfoldDNdetaProj->Write();
  hMissMultProj->Write();
  hFakeTrainMultProj->Write();
  hFeedInTrainMultProj->Write();
  hRecoAllTrainMultProj->Write();
  hRecoDataUnfoldMultProj->Write();
  hMatchedTrainMultProj->Write();
  hEfficiencyMultProj->Write();
  hLostSignalMultProj->Write();
  hLostSignalFractionMultProj->Write();
  hLostCollisionMultProj->Write();
  hCollisionEfficiencyMultProj->Write();
  hLostCollisionFractionMultProj->Write();
  hRatioPt->Write();
  hRatioMult->Write();
