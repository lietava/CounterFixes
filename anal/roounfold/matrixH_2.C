...
// useAO2Data: 1 = read real reco data from AO2Ddata.root; 0 = generate independent toy data; 2 = use RecoTrain closure input.
void matrixH(TString trainAO2D = "AO2D.root", bool useAO2DTraining = true,
             TString dataAO2D = "AO2Ddata.root", int useAO2Data = 1)
{
  ...
  // ------------------------------------------------------------------
  // 4. DATA SAMPLE
  // useAO2Data = 1: read real reco data from AO2Ddata.root
  //             0: generate independent pseudo-data
  //             2: use RecoTrain for closure/debug
  // ------------------------------------------------------------------
  if (useAO2Data == 1) {
    std::cout << "Reading real data sample from " << dataAO2D << std::endl;
    if (dataAO2D.IsNull()) {
    ...
    std::cerr << "Unknown useAO2Data option " << useAO2Data
              << ": use 1 for AO2Ddata, 0 for generated data, 2 for RecoTrain" << std::endl;
    return;
  }
