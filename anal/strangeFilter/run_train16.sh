...
### Creating necessary json files and constructing train test command
  # Reading JSON file
    ...
    pipeline_strategies=`echo $workflows | jq -r '[.[] | .pipeline_strategy // {}]'`
...
if [[ -n "$IO_RATE_LIMIT" &&  ( "$O2_PACKAGE" > "VO_ALICE@O2Physics::daily-20241010-0200-1" || "$O2_PACKAGE" == "VO_ALICE@O2Physics::eulisse-local" ) ]]; then
  ## Test JF, 21.05.25
  if [[ "$ALIEN_JDL_HYTRAIN" -eq "408602" ]]; then
    IO_RATE_LIMIT=7
  fi
...
EARLY_FORWARDING_POLICY=""
if [[ $cpu_cores -gt 1 ]]; then
  EARLY_FORWARDING_POLICY="--early-forward-policy noraw"
fi
...
WORKFLOW_OPTIONS="$WORKFLOW_OPTIONS_SIMPLE $SESSION $TIMEFRAMELIMIT $EXTRAARGS $EARLY_FORWARDING_POLICY"
...
### Generating the command
  ...
  for (( i=0; i<$total_tests ; i++ )); do
    ...
    pipeline_strategy_obj=`echo "$pipeline_strategies" | jq -r ".[$i]"`
    pipeline_strategy=$(echo "$pipeline_strategy_obj" | jq -r 'to_entries | map("\(.key):\(.value)") | join(",")')
    if [ -n "$pipeline_strategy" ] && [ "$pipeline_strategy" != "" ]; then
...
# Resolve duplicated workflows (same workflow_name with different wagon_ids)
workflow_id = {}
for workflow in train["workflows"]:
  if not workflow["workflow_name"] in workflow_id:
    workflow_id[workflow["workflow_name"]] = []
  if not workflow["wagon_id"] in workflow_id[workflow["workflow_name"]]:
    workflow_id[workflow["workflow_name"]].append(workflow["wagon_id"])

for workflow in train["workflows"]:
  name = workflow["workflow_name"]
  if len(workflow_id[name]) > 1:
    name += "_id" + str(workflow["wagon_id"])
  # print("W", name)
  if not name in aggregated:
...
# Calculate average timeframes in fly from driver metrics
average_timeframes_in_fly = 0
if "driver" in data and "total-timeframes-in-fly" in data["driver"]:
  tf_in_fly_data = data["driver"]["total-timeframes-in-fly"]
  if tf_in_fly_data:
    values = [float(elem["value"]) for elem in tf_in_fly_data]
    if values:
      average_timeframes_in_fly = sum(values) / len(values)

output["__full_train__"]["average_timeframes_in_fly"] = [ average_timeframes_in_fly ]
print("Average timeframes in fly: %.2f"%average_timeframes_in_fly)
