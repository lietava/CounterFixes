...
void uploadOTSobjects()
{
  o2::ccdb::CcdbApi api;
  api.init("http://alice-ccdb.cern.ch");
  std::string path = "/home/rl/CounterFixes/trigger/2240";
  std::map<std::string, std::string> metadata;
  std::string runString = "536663";
  std::cout << "Processing run " << runString << std::endl;
  const int runNumber = std::stoi(runString);
  metadata["runNumber"] = "2240";
  ...
  TFile scalersFile((path + "/AnalysisResults.root").data(), "READ");
  ...
  TH1* filters = (TH1*)scalersFile.Get("central-event-filter-task/scalers/mFiltered");
    ...
    TFile bcRangesFile((path + "/bcRanges.root").data(), "READ");
    int Nmax = 0;
