...
struct Hists
{
  ...
  TH1F* addHist(std::string const&name,int n, int n1, int n2)
  {
  ...
  TH1F* addHistWithNames(std::string const&name)
  {
    TH1F *h = new TH1F(name.c_str(), name.c_str(), Ndim, 0, Ndim);
...
void bcInfo::print() const
{
  *mylog << bcEvSel << " " << bcAOD << " " << std::hex << trigMask << " " << selMask << " " << std::dec;
  for(size_t i = 0; i < Ndim; i++) {
    ...
    if(mask & trigMask) {
      if(i < Hists::labels.size()) {
        *mylog << Hists::labels[i] << " ";
      } else {
        *mylog << "XXX ";
      }
    }
...
void bcInfos::getDataCCDB(int runNumber, int Nmax, bool print)
{
  ...
  *mylog << " SBC size:" << (*sbc).size() << std::endl;
  ...
  if(print) {
    for(int i = 0; i < Ndim; i++) {
      *mylog << i << " " << triggerCounters[i] << " " << mFilterCounters->GetBinContent(i+2) << "||" << selectionCounters[i] << " " << mSelectionCounters->GetBinContent(i+2) << std::endl;
    }
  }
}
...
void bcInfos::selectionEfficiency( std::array<double_t,Ndim>& eff,std::array<double_t,Ndim>& err )
{
  *mylog << "===> Selection efficiency - downscaling" << std::endl;
  ...
  for(int i =0; i < Ndim; i++) {
    ...
    std::string label = "?";
    if(i < (int) Hists::labels.size()) {
      label = Hists::labels[i];
    }
...
struct effUtils : Hists
{
  ...
  std::array<double_t,Ndim> effWin{0};
  ...
  void skimmedEfficiencyWindow();
  void skimmedEfficiencyWindow2();
...
void effUtils::readFiles(TFile& file, int n = 0)
{
  ...
  originalBCs.getDataCCDB(runNumber, n,0);
}
...
void effUtils::skimmedEfficiencyWindow()
{
  int window = 100;
  *mylog << "eff started with window:" << window << std::endl;
  std::map<uint64_t,uint64_t> bctomask;
  for(auto const& bci: skimmedBCs.bcs) {
    uint64_t bc = bci.bcEvSel;
    if(bci.trigMask) {
      for(int j = 0; j < 2*window-1 ; j++) {
        uint64_t bc1 = bc - window + j;
        if(bctomask.count(bc1)) {
          uint64_t mask = bci.trigMask | bctomask[bc1];
          bctomask[bc1] = mask;
        } else {
          bctomask[bc1] = bci.trigMask;
        }
      }
  }
  }
  *mylog << "bctomask created:" << bctomask.size() << std::endl;
  std::array<int, Ndim> triggerWinCounters{0};
  for(auto const& bc: originalBCs.bcs) {
    for(int i = 0 ; i < Ndim; i ++) {
      uint64_t mask = 1ull << i;
      if(mask & bc.selMask) {
        uint64_t bce = bc.bcEvSel;
        if(bctomask.count(bce)) {
          if(mask & bctomask[bce]) {
            triggerWinCounters[i]++;
          }
        }
      }
    }
  }
  TH1F *h = addHistWithNames("Skimming efficiency");
  int *before = originalBCs.selectionCounters.data();
  for(int i = 0; i < Ndim; i++) {
    double_t eff = (double_t) triggerWinCounters[i]/before[i];
    effWin[i] = eff;
    double_t err = sqrt(eff*(1-eff)/before[i]);
    *mylog << i << " before:" << before[i] << " after:" << triggerWinCounters[i] << " effWin:" << effWin[i] << std::endl;
    h->SetBinContent(i+1,effSkimmed[i]);
    h->SetBinError(i+1,effSkimmedError[i]);
  }
  printSkimmedEff();
}
void effUtils::skimmedEfficiencyWindow2()
{
  std::map<uint64_t,uint64_t> svb;
  for(auto const& bci: skimmedBCs.bcs) {
    svb[bci.bcEvSel] = bci.trigMask;
  }
  uint64_t mask;
  std::array<int, Ndim> triggerWinCounters{0};
  for(auto const& bco: originalBCs.bcs) {
    //int ind = lower_bound(sv.begin(),sv.end(),bco
    if(svb.count(bco.bcEvSel)) {
      for(int i = 0; i < Ndim; i++){
        mask = 1ull << i;
        if(mask  & bco.selMask) {
          if(mask & svb[bco.bcEvSel]) {
            triggerWinCounters[i]++;
          }
        }
      }
...
void effUtils::printSkimmedEff()
{
  ...
  TH1F *h = addHistWithNames("Skimming eff");
  ...
  *mylog << "===> Skimming efficiency" << std::endl;
  for(int i = 0; i < Ndim; i++) {
    ...
    double_t venn_norm = 0;
    if(before[i]) {
      venn_norm = (double_t)vennInter[i]/before[i];
    }
    *mylog << std::fixed << std::setprecision(3);
    if(originalBCs.downscaleFactors[i] < 0.99 && before[i] != 0) {
      //*mylog << "DDDDD ";
      //continue;
    }
    effSkimmed[i] = effWin[i];
    if(effSkimmed[i] > 1) *mylog << ">>>>> ";
    //if(effSkimmed[i] < 0.6) continue;
    *mylog << "BIT:" << std::setw(2) << i << "  " << std::setw(25) << label << " ds:" << std::setw(6) << originalBCs.downscaleFactors[i] << "  b:" << std::setw(10) << before[i] << " a:" << std::setw(10) << after[i] << " eff:" << std::setw(8) << effSkimmed[i] << " VennInter:" << std::setw(9) << vennInter[i] << " Inter/before:" << std::setw(6) << venn_norm << " singles frac b/a:" << std::setw(6) << (double_t)originalBCs.freqDSel[i]/before[i] << " " << std::setw(6) << (double_t)skimmedBCs.freqDTri[i]/after[i]  <<  std::endl;
    h->SetBinContent(i+1,effSkimmed[i]);
    h->SetBinError(i+1,effSkimmedError[i]);
  }
}
void effUtils::getFrequencyComp()
{
  ...
  TH1F *h1 = addHistWithNames("Original Singles Freq Selected");
  h1->SetAxisRange(0,1.2,"Y");
  TH1F *h2 = addHistWithNames("Skimmed Singles Freq Triggered");
  h2->SetAxisRange(0,1.2,"Y");
...
void effUtils::vennDiagram()
{
  ...
  TH1F *h = addHistWithNames("Venn intersection of Trig and Sel");
  h->SetAxisRange(0,1.2,"Y");
  *mylog << "Venn diagrams::" << std::endl;
  for(int i = 0; i< Ndim; i++) {
    *mylog << "[" << i << "]" << originalBCs.selectionCounters[i] - vennInter[i] << " " << vennInter[i] << " " << skimmedBCs.triggerCounters[i] - vennInter[i] << " " << std::endl;
    double_t e = 0;
    double_t er = 0;
    if(originalBCs.selectionCounters[i]) {
      e = (double_t) vennInter[i]/originalBCs.selectionCounters[i];
      er = sqrt(e*(1-e)/originalBCs.selectionCounters[i]);
    }
    h->Fill(i,e);
    h->SetBinError(i+1,er);
  }
}
void effUtils::vennDiagram(int i)
{
  if(i > Ndim) {
    vennDiagram();
  }
  ...
  *mylog << "Venn [" << i << "]" << originalBCs.selectionCounters[i] - vennInter[i] << " " << vennInter[i] << " " << skimmedBCs.triggerCounters[i] - vennInter[i] << " " << std::endl;
}
...
void effUtils::printDownscaleFactors()
{
  ...
  TH1F *h = addHistWithNames("Downscale Factors");
  h->SetAxisRange(0,1.2,"Y");
