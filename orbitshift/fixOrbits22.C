void fixOrbits22() {
  //std::vector<int> runNbVect = {523141, 523142};
  ...
  for (int runNb : runNbVect) {
    ...
    std::cout << " Orbit reset " << oresetTime << " Time first scalers " << tlr.first << " ccdb: " << pp.first << " scalers last:" << tlr.second << std::endl;
    ...
    if (tlr.first >= oresetTime) {
      ...
      std::cout << " scaler time > orbit reset time, skipping" << std::endl;
      continue;
    }
    ...
    auto headersGRP = ccdb.getSpecific<o2::parameters::GRPECSObject>("GLO/Config/GRPECS", tstart + 2000, metadata);
    ...
    std::cout << "Writing to CCDB startValGRP:" << startValGRP << " endValGRP:" << endValGRP << std::endl;
    std::cout << "Comparison to scalers first: " <<  (int)(-startValGRP + tlr.first) << " last: " << (int)(endValGRP - tlr.second) << std::endl;
    // write to ccdb
